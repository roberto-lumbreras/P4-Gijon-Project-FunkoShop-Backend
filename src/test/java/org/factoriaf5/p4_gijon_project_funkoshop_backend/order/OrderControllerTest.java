...
public class OrderControllerTest {
    ...
    void setup() {
        ...
        products.add(productRepository.save(new Product(null, "Laptop", "Gaming Laptop", new BigDecimal(1000), 5, 5, null, null, categories.get(0), LocalDateTime.now().truncatedTo(ChronoUnit.SECONDS))));
        products.add(productRepository.save(new Product(null, "Shirt", "Cotton Shirt", new BigDecimal(20), 50, 50, null, null, categories.get(1), LocalDateTime.now().truncatedTo(ChronoUnit.SECONDS))));
        products.add(productRepository.save(new Product(null, "Book", "Java Programming", new BigDecimal(30), 15, 15, null, null, categories.get(2), LocalDateTime.now().truncatedTo(ChronoUnit.SECONDS))));
    
        users.add(userRepository.save(new User(null, "admin", "admin@mail.com", passwordEncoder.encode("password"), true, Role.ROLE_ADMIN, "token1", products)));
        users.add(userRepository.save(new User(null, "customer1", "customer1@mail.com", passwordEncoder.encode("password"), true, Role.ROLE_USER, "token2", List.of())));
        users.add(userRepository.save(new User(null, "customer2", "customer2@mail.com", passwordEncoder.encode("password"), true, Role.ROLE_USER, "token3", List.of())));
        ...
        orders.add(orderRepository.save(new Order(null, users.get(0), LocalDate.now().minusMonths(6), true, "Credit Card", Order.Status.PENDING, totalAmount, productQuantity, productListOrderOne)));
        ...
        orders.add(orderRepository.save(new Order(null, users.get(1), LocalDate.now().minusMonths(1), false, "Credit Card", Order.Status.PENDING, totalAmount, productQuantity, productListOrderTwo)));
        ...
        orders.add(orderRepository.save(new Order(null, users.get(2), LocalDate.now(), true, "Credit Card", Order.Status.DELIVERED, totalAmount, productQuantity, productListOrderThree)));        
    }
    ...
    void testGetAllOrders() throws Exception{
        ...
        .andExpect(MockMvcResultMatchers.content().json(mapper.writeValueAsString(orders.stream().map(OrderDTO::new).collect(Collectors.toList()))));
    }
    ...
    void testGetBestSellers() throws Exception{
        ...
        .with(SecurityMockMvcRequestPostProcessors.user("admin").password("password").roles("ADMIN")))
        .andExpect(MockMvcResultMatchers.status().isOk())
        .andExpect(MockMvcResultMatchers.content().json(mapper.writeValueAsString(List.of(new ProductDTO(products.get(0)),new ProductDTO(products.get(2)),new ProductDTO(products.get(1))))));
    }
    ...
    void testListByMonth() throws Exception{
        ...
        .with(SecurityMockMvcRequestPostProcessors.user("admin").password("password").roles("ADMIN")))
        .andExpect(MockMvcResultMatchers.status().isOk())
        .andExpect(MockMvcResultMatchers.content().json(mapper.writeValueAsString(List.of(new OrderDTO(orders.get(1))))));
    }
    ...
    void testUpdateStatus() throws Exception{
        ...
        .andExpect(MockMvcResultMatchers.status().isOk());
        assertTrue(orderRepository.findById(orders.get(0).getOrderId()).get().getStatus().name().equals(Order.Status.DELIVERED.name()));
    }
}
