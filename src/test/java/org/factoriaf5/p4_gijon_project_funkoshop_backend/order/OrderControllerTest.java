...
public class OrderControllerTest {
        ...
        void setup() {
                ...
                products.add(productRepository.save(new Product(null, "Laptop", "Gaming Laptop", new BigDecimal(1000),
                                5, 5,
                                null, null, categories.get(0), LocalDateTime.now().truncatedTo(ChronoUnit.SECONDS))));
                products.add(productRepository.save(new Product(null, "Shirt", "Cotton Shirt", new BigDecimal(20), 50,
                                50, null,
                                null, categories.get(1), LocalDateTime.now().truncatedTo(ChronoUnit.SECONDS))));
                products.add(productRepository.save(new Product(null, "Book", "Java Programming", new BigDecimal(30),
                                15, 15,
                                null, null, categories.get(2), LocalDateTime.now().truncatedTo(ChronoUnit.SECONDS))));

                users.add(userRepository
                                .save(new User(null, "admin", "admin@mail.com", passwordEncoder.encode("password"),
                ...
                productListOrderOne.add(
                                new DetailOrder(null, products.get(0), 2, products.get(0).getPrice().doubleValue()));
                Float totalAmount = (float) productListOrderOne.stream()
                ...
                orders.add(orderRepository.save(new Order(null, users.get(0), LocalDate.now().minusMonths(6), true,
                                "Credit Card", Order.Status.PENDING, totalAmount, productQuantity,
                                productListOrderOne)));

                List<DetailOrder> productListOrderTwo = new ArrayList<>();
                productListOrderTwo.add(
                                new DetailOrder(null, products.get(0), 1, products.get(0).getPrice().doubleValue()));
                productListOrderTwo.add(
                                new DetailOrder(null, products.get(1), 1, products.get(1).getPrice().doubleValue()));
                totalAmount = (float) productListOrderTwo.stream().mapToDouble(x -> x.getPrice() * x.getQuantity())
                                .sum();
                ...
                productListOrderThree.add(
                                new DetailOrder(null, products.get(0), 1, products.get(0).getPrice().doubleValue()));
                productListOrderThree.add(
                                new DetailOrder(null, products.get(2), 2, products.get(2).getPrice().doubleValue()));
                totalAmount = (float) productListOrderThree.stream().mapToDouble(x -> x.getPrice() * x.getQuantity())
                                .sum();
        ...
        void testGetAllOrders() throws Exception {
                mockMvc.perform(MockMvcRequestBuilders.get("/orders")
                                ...
                                .andExpect(MockMvcResultMatchers.content().json(
                                                mapper.writeValueAsString(orders.stream().map(OrderDTO::new)
        ...
        void testListOrdersByUser() throws Exception {
                mockMvc.perform(MockMvcRequestBuilders.get("/orders/user/{id}", users.get(1).getId())
                                .with(SecurityMockMvcRequestPostProcessors.user("customer1").password("password")
                                                .roles("USER")))
                                .andExpect(MockMvcResultMatchers.status().isOk())
                                .andExpect(MockMvcResultMatchers.content()
                                                .json(mapper.writeValueAsString(List.of(new OrderDTO(orders.get(1))))));
        ...
        void testGetStatus() throws Exception {
                mockMvc.perform(MockMvcRequestBuilders.get("/orders/order/status/{orderId}", orders.get(0).getOrderId())
                                .with(SecurityMockMvcRequestPostProcessors.user("admin").password("password")
                                                .roles("ADMIN")))
                                .andExpect(MockMvcResultMatchers.status().isOk());
        }
        ...
        @Test
        void testGenerateOrderPDFId() throws Exception {
                mockMvc.perform(MockMvcRequestBuilders.get("/orders/order/pdf/{orderId}", orders.get(0).getOrderId())
                                .with(SecurityMockMvcRequestPostProcessors.user("admin").password("password")
                                                .roles("ADMIN")))
                                .andExpect(MockMvcResultMatchers.status().isOk());
        }
        ...
        @Test
        void testGenerateOrderPDF() throws Exception {
                mockMvc.perform(MockMvcRequestBuilders.get("/orders/details/pdf")
                                .with(SecurityMockMvcRequestPostProcessors.user("admin").password("password")
                                                .roles("ADMIN")))
                                .andExpect(MockMvcResultMatchers.status().isOk());
        }
