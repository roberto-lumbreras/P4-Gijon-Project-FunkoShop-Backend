 ...
 * 
 * import
 * org.factoriaf5.p4_gijon_project_funkoshop_backend.order.OrderService.Status;
 ...
 * createOrder(@RequestHeader("Authorization") String authorizationHeader,
 * 
 ...
 * getAllOrders(@RequestHeader("Authorization") String authorizationHeader) {
 * try {
 * if (authorizationHeader == null ||
 * !authorizationHeader.startsWith("Bearer ")) {
 * return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
 ...
 * }
 * 
 * List<OrderDto> orderList = orderService.listOrdersByUser(authorizationHeader,
 * orderDto);
 * 
 * return ResponseEntity.ok(orderList);
 * } catch (IllegalArgumentException error) {
 ...
 * public ResponseEntity<Map<String, String>>
 * updateStatus(@RequestHeader("Authorization") String authorizationHeader,
 * 
 ...
 * headers.setContentDisposition(ContentDisposition.inline().filename("order_" +
 * orderId + ".pdf").build());
