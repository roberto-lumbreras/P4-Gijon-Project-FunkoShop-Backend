...
public class DetailOrderController {
    ...
    public DetailOrderController(DetailOrderService detailOrderService, DetailOrderDto orderDto) {
    ...
    public ResponseEntity<List<DetailOrderDto>> retrieveSales(
        ...
        try {
            ...
            List<DetailOrderDto> salesList = detailOrderService.getSales(authorizationHeader);
    ...
    public ResponseEntity<byte[]> generateOrderPDF(@RequestHeader("Authorization") String authorizationHeader, DetailOrderDto detailOrderDto) {
        try {
            if (authorizationHeader == null || !authorizationHeader.startsWith("Bearer ")) {
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
            }
            byte[] pdfData = detailOrderService.generatePDFAllOrders(authorizationHeader, detailOrderDto);
    ...
    public ResponseEntity<Integer> CalculateQuantity(
        @RequestHeader("Authorization") String authorizationHeader,DetailOrderDto detailOrderDto) {
            try {
                ...
                Integer quantity = detailOrderService.calculateProductQuantity(authorizationHeader, detailOrderDto);
