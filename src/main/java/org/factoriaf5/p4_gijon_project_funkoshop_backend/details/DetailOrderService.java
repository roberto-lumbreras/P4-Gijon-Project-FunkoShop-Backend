 ...
 * import
 * org.factoriaf5.p4_gijon_project_funkoshop_backend.order.OrderRepository;
 ...
 * import
 * org.factoriaf5.p4_gijon_project_funkoshop_backend.product.ProductRepository;
 ...
 * .orElseThrow(() -> new
 * IllegalArgumentException("Usuario del token no encontrado"));
 * 
 * if (!token.equals(user.getJwToken()) && !"admin".equals(user.getRole())) {
 * throw new
 * SecurityException("Acceso denegado. Solo el ADMIN puede realizar la acción");
 * }
 * 
 ...
 * return !orderDate.isBefore(firstDayOfLastMonth) &&
 * !orderDate.isAfter(lastDayOfLastMonth);
 ...
 * 
 * productSalesMap.put(productId, productSalesMap.getOrDefault(productId, 0) +
 * quantity);
 * }
 * }
 * 
 ...
 * 
 * List<Map.Entry<Long, Integer>> sortedProductList =
 ...
 * }
 * 
 * public byte[] generatePDFAllOrders(String authorizationHeader, DetailOrderDto
 * detailOrderDto) {
 * 
 ...
 * }
 * 
 * public Integer calculateQuantity(String authorizationHeader, Order orderId,
 * Product productId) {
 * 
 * String token = authorizationHeader.substring(7);
 * String emailToken = jwtUtils.getEmailFromJwtToken(token);
 ...
 * int quantity = detailOrderDto.getProductQuantity();
 * 
 * quantityMap.put(productId, quantityMap.getOrDefault(productId, 0) +
 * quantity);
 * 
 * }
 * }
